...
if ($_POST['accion'] == "ListarClientes") {
    ...
    if (!empty($arrCliente)) {
        for ($i = 0; $i < count($arrCliente); $i++) {
            ...
            $eliminar = '<button class="btn btn-icon btn-sm btn-danger" onClick="EliminarCliente(\'' . $arrCliente[$i]['numero'] . '\')" title="Eliminar Cliente"><i class="bx bxs-trash"></i></button>';

            $arrCliente[$i]["opciones"] = '<div class="text-center">' . $editar . ' ' . $activo . ' ' . $inactivo . ' ' . $eliminar . '</div>';
    ...
    if (empty($documento) || empty($numero) || empty($pnombre) || empty($papellido) || empty($fechan) || empty($sexo)) {
        ...
        try {
            ...
            if ($id_registro > 0) {
                ...
                $respuesta = array(
                    'codigo' => 1,
                    'respuesta' => 'No se Logro Registrar el Cliente',
                );
            }
            $ejecutar->close();
            $mysqli->close();
...
if ($_POST['accion'] == 'EliminarCliente') {
    $numero = $_POST['db_cliente'];  // Recibe el documento de cliente a eliminar

    require_once('../config/conexion.php');

    $query = "DELETE FROM db_clientes WHERE numero = ?";  // Uso de 'documento' para eliminar
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("s", $numero);  // 's' es el tipo de parámetro (string)

    if ($stmt->execute()) {
        echo json_encode(['codigo' => 0, 'respuesta' => 'Cliente eliminado correctamente.']);
    } else {
        echo json_encode(['codigo' => 1, 'respuesta' => 'Error al eliminar el cliente: ' . $stmt->error]);
    }
